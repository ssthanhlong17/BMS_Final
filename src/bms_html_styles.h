...
function updateProtectionStatus(protectionData) {
    const protections = {
        ...
        'protectOTCharge': { 
            data: protectionData.overTempCharge, 
            label: 'Over Temp (Charge)' 
        },
        'protectOTDischarge': { 
            data: protectionData.overTempDischarge, 
            label: 'Over Temp (Discharge)' 
        },
        'protectUTCharge': { 
            data: protectionData.underTempCharge, 
            label: 'Under Temp (Charge)' 
        },
        'protectUTDischarge': { 
            data: protectionData.underTempDischarge, 
            label: 'Under Temp (Discharge)' 
        }
    };
...
function updateAlerts(alerts) {
    ...
    container.innerHTML = alerts.map(alert => {
        const icon = alert.icon || (alert.severity === 'critical' ? '🚨' : '⚠️');
...
async function fetchBMSData() {
    try {
        ...
        if (data.calculation?.remainingCapacity) {
            const capacity = parseFloat(data.calculation.remainingCapacity).toFixed(2);
            updateElement('remainingCapacity', capacity, ' Ah');
        }
        
        if (data.calculation?.totalCycles) {
            const cycles = parseFloat(data.calculation.totalCycles).toFixed(0);
            updateElement('totalCycles', cycles);
        }
        ...
        // ===== BALANCING - Chỉ hiển thị trạng thái từ server =====
        if (data.status?.balancing) {
            ...
            // Cập nhật cell display với trạng thái balancing
            if (data.measurement?.cellVoltages) {
                ...
                if (display && display.innerHTML !== batteryHTML) {
        ...
        const display = document.getElementById('batteryDisplay');
        if (display) {
            display.innerHTML = '<div class="loading">⚠️ Connection Error</div>';
        }
    }
}
...
(function init() {
    ...
    // Thêm transition cho các element
    const animatedElements = [
        'packVolt', 'soh', 'current', 'packTemp', 
        'batteryDisplay', 'socPercentage', 'socCircle',
        'remainingCapacity', 'totalCycles', 'balancingStatus'
    ];
